...
日期：2026-08-28  
状态：已实施；2026-09-07 完成当前机器厂商卸载验收
...
用户点击“打开联想官方卸载程序”并确认后，短生命周期的管理员 clean 进程在启动任何第三方程序前重新读取同一个卸载注册表值，并执行以下失败关闭检查。GUI 保留的旧结果兼容路径必须在隔离 PowerShell 进程中执行同一复验，不能直接依赖长驻 WPF 运行空间：
...
若复验失败，结果为 `skipped`、`failure_stage` 为空，并提示重新扫描。若经过复验但进程启动 API 失败，内部 handoff 结果为 `failed/launch`；写入统一 clean 终态时映射为 `failed/mutation`，只展示净化后的固定原因。
...
- GUI：展示新动作、执行用户确认、生成并绑定执行子集与影响摘要，严格读取终态结果；仅为旧 sentinel 结果保留隔离进程兼容交接。
- ActionEngine：不再尝试对 PPL HRWSCCtrl 执行 STOP；在管理员 clean 进程中完成官方卸载最终复验与 EXE 启动，现有非 PPL 服务停止合同不变。
...
- 启动 API 故障：handoff 内部为 `failed/launch`，统一 clean 终态为 `failed/mutation`；不泄漏路径、命令行、token 或原始异常。
...
2026-09-07 的用户批准验收已证明：可信动作能够打开本机联想 `uninst.exe`，Cleaner 没有传静默参数、没有自动点击，也没有把打开卸载器标为成功；用户随后在联想界面完成卸载，0 / 5 / 30 秒回读均未发现服务、目标进程或卸载项。管理员 clean 直接交接是在目标卸载后补入，已完成自动回归，但无法在同一已消失目标上重复实机启动。
